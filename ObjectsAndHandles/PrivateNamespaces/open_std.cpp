// open_std.cpp
//
// Open a private namespace accessible to all standard users.
//
// Build
//  cl /EHsc /nologo /std:c++17 /W4 /I %WIN_WORKSPACE%\_Deps\WDL\include open_std.cpp
...
int main()
{
    ...
    auto ns = private_namespace_handle{
        ::OpenPrivateNamespaceW(bd.get(), L"Private") };
    if (!ns)
    {
        error("OpenPrivateNamespace() failed");
        return STATUS_FAILURE_I;
    }
    ...
    std::cout << "[+] Succesfully opened named mutex in private namespace\n";
    std::cin.get();
