...
//
// Build
//	cl /EHsc /nologo /std:c++17 /W4 /I %WIN_WORKSPACE%\_Deps\WDL\include performance_test.cpp
...
void run_critical_section_test(unsigned long n_threads)
{
	...
	{
		wdl::timing::scoped_timer t{ "Critical Section Test" };
...
void run_mutex_test(unsigned long n_threads)
{
	...
	{
		wdl::timing::scoped_timer t{ "Mutex Test" };
...
void run_interlocked_test(unsigned long n_threads)
{
	...
	{
		wdl::timing::scoped_timer t{ "Interlocked Test" };
